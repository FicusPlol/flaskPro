...
def now_user():
    """
    общая информация о юзере доступная во всех вкладках и темплейтах
...
def user_edit(id):
    """
    Открывает страницу для редактирования данных пользователя, при сохранении меняет информацию о профиле,
    в случае замены аватара, предыдущее фото удаляется и сохраняется новое, (исключение: def.jpg- фото по умолчанию)
    :param id: id юзера
